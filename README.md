...
- 单从一个层面考虑性能不高明，比如为了减少GC而大块分配内存，却提供`[]byte`接受`value`，意味着必须先序列化（有碎片）
- 《第三层次》里有一句“没有比不存在的东西性能更快的了”，额外操作虽不在性能指标里，实际用还是要算，而`cache`直接省了这部分
- 更费解的是，有的library甚至还包了server🤔️那我为什么不用更全面、稳定的redis，感觉忘了初心
