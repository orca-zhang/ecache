...
- 《第三层次》里有一句“没有比不存在的东西性能更快的了”（类似奥卡姆剃刀原理），能砍掉的东西一定不要想着优化
- 比如为了减少GC大块分配内存，却提供`[]byte`的值存储，意味着必须先序列化（有碎片）再拷贝，额外操作虽不在性能指标里，实际用还是要算
- 如果序列化的部分可以复用用在协议层拼接，能做到`ZeroCopy`，那也无可厚非，而`cache`存储指针直接省了额外的部分
- 有的library甚至还包了server🤔️🤔️🤔️请问我为什么不用更全面、稳定的redis（一键安装），感觉忘了初心
